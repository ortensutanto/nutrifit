...
export default function HomeScreen() {
  ...
  const fetchGoalData = async () => {
    try {
      ...
      const goalType = (() => {
        switch ((userData.primaryGoal || '').toLowerCase()) {
          case 'lose weight': return 1;
          case 'gain weight': return 2;
          default: return 3;
        }
      })();
      ...
      await axios.post(`${apiURL}/goals/calculateGoals`, {
        goal_type: goalType,
    ...
    const goalInfoResponse = await axios.get(
      `${apiURL}/goals/getGoalInfo`,
      { headers: { Authorization: `Bearer ${token}` } }
    );
    ...
    const { goal_calories, consumed_calories, calories_deficit } = calorieResponse.data;
    ...
    console.error('Error fetching goal data:', axiosError.response?.data || axiosError.message);
  } finally {
    setLoading(false);
  }
    ...
    <ScrollView style={styles.container}>
      <View style={styles.headerContainer}>
        <View>
          <Text style={styles.welcomeText}>
            {isHydrated ? `Welcome back, ${userData.displayName || 'User'}!` : 'Welcome back...'}
          </Text>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.calorieContent}>
          ...
          <View style={styles.calorieBreakdown}>
            <Text style={styles.breakdownText}>🎯 Base Goal : {goalData.goalCalories}</Text>
            <Text style={styles.breakdownText}>🍽️ Food : {goalData.consumedCalories}</Text>
            <Text style={styles.breakdownText}>🔥 Exercise : {goalData.exerciseCalories}</Text>
          </View>
        </View>
        {loading && (
      ...
      <View style={styles.bottomRow}>
        ...
        <View style={styles.metricBox}>
          ...
          <Text style={styles.metricValue}>{exerciseData.caloriesBurned} kcal</Text>
          <Text style={styles.metricLabel}>Exercise</Text>
          <Text style={styles.subLabel}>{exerciseData.durationMinutes} minutes</Text>
        </View>
      </View>
    </ScrollView>
  );
