    ...
    try {
        ...
        const recipeRequest = await transaction.execute(`
            ...
            (?, ?, ?, ?, ?,
                ?, ?, ?,
                ?, ?, ?)
        `, [newRecipeId, '83d6d048-06ab-41f0-9cdf-0c26e8331aab', imageUrl, recipe_name, recipe_description,
            serving_size, recipeCalories, instructionString,
            prepTime, cookTime, new Date()]);
        ...
        for (const ing of ingList) {
            ...
            const findReq = await transaction.execute('SELECT food_item_id from NutriFit.food_items WHERE name = ?', [name]);
            ...
            if (findReq[0].length) {
                ...
                const ingredientRequest = await transaction.execute(`
                    ...
                    VALUES
                        (?, ?, ?, ?, ?, ?)
                `, [foodItemId, ingredientDetails.foodName, ingredientDetails.calories, ingredientDetails.protein, ingredientDetails.carbohydrates, ingredientDetails.fat]);
            }
