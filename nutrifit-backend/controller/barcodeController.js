...
export async function scanBarcodeAPI(req, res) {
    ...
    try {
        ...
        // Staging API
        const API_URL = `https://world.openfoodfacts.net/api/v2/product/${String(barcodeScan)}?fields=product_name,nutriments`;
        ...
        if (!response.ok) {
            ...
            return res.status(response.status).json({ error: `Failed to fetch data from Open Food Facts: ${response.statusText}`, details: errorText });
        }
        ...
        if (data.status === 0 || data.status == 0) {
            ...
            return res.status(404).json({ error: "Product not found on Open Food Facts API" });
        }

        const food_item_id = uuidv4();
        const name = data.product.product_name;
        const calories = data.product.nutriments['energy-kcal_serving'] || data.product.nutriments['energy-kcal'] || 0;
        const protein = data.product.nutriments.proteins_serving || data.product.nutriments.proteins || 0;
        const carbohydrate = data.product.nutriments.carbohydrates_serving || data.product.nutriments.carbohydrates || 0;
        const fat = data.product.nutriments.fat_serving || data.product.nutriments.fat || 0;
        await addToFoodItemDatabase(food_item_id, barcodeScan, name, calories, protein, carbohydrate, fat);
