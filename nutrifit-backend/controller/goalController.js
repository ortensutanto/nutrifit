...
export async function calculateGoals(req, res) {
  try {
    ...
    if (goalType === 1 || goalType === 2) {
      ...
      calorieDeficit =
        ((targetWeightChange / 0.45) * 3500) / (targetTimeWeeks * 7);
    }
...
export async function copyPreviousGoal(req, res) {
  try {
    ...
    if (!goalSearch[0] || goalSearch[0].length === 0) {
      return res
        .status(404)
        .json({ error: "No previous day's goal found to copy" });
    }
    ...
    if (existingGoal[0] && existingGoal[0].length > 0) {
      return res.status(400).json({
        error: "Goal for today already exists.",
        goal_id: existingGoal[0].goal_id,
      });
    }
    ...
    return res.status(201).json({
      message: "Previous goal succesfully copied to today",
      goal_id: goal_id,
    });
  } catch (err) {
    console.error(err);
...
export async function getTodayGoal(req, res) {
  try {
    ...
    const userVerification = await connection
      .promise()
      .query("SELECT user_id FROM NutriFit.user WHERE user_id = ?", [userId]);
...
export async function getGoalInfo(req, res) {
  try {
    ...
    const [goals] = await connection
      .promise()
      .query("SELECT * FROM NutriFit.goals WHERE user_id = ?", [userId]);
...
export async function getCalorieNeeded(req, res) {
  try {
    ...
    const [goals] = await connection
      .promise()
      .query(`SELECT target_calories_per_day FROM goals WHERE goal_id=?`, [
        goalId,
      ]);
